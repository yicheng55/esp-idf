...
#include "sdkconfig.h"
#if defined(CONFIG_EXAMPLE_PTP_TRANSPORT_IPV4) && CONFIG_EXAMPLE_PTP_TRANSPORT_IPV4
#include "esp_netif.h"
#include "esp_netif_ip_addr.h"
#define ESP_PTP_UDP_IPV4 1
#else
#define ESP_PTP_UDP_IPV4 0
#endif
#if defined(ESP_PTP) && !ESP_PTP_UDP_IPV4
#define ESP_PTP_USE_L2TAP 1
#else
#define ESP_PTP_USE_L2TAP 0
#endif

#if defined(CONFIG_NETUTILS_PTPD_IEEE_802_1AS) && CONFIG_NETUTILS_PTPD_IEEE_802_1AS && ESP_PTP_UDP_IPV4
#error "IEEE 802.1AS (gPTP) on ESP requires L2 transport; disable CONFIG_EXAMPLE_PTP_TRANSPORT_IPV4"
#endif
...
#if ESP_PTP_UDP_IPV4
static esp_eth_handle_t g_ptpd_eth_handle;
#endif
#endif
...
#if ESP_PTP_UDP_IPV4
void ptpd_register_eth_handle(esp_eth_handle_t eth)
{
  g_ptpd_eth_handle = eth;
}
#endif

#if ESP_PTP_USE_L2TAP
...
#endif /* ESP_PTP_USE_L2TAP */

#endif /* ESP_PTP (helpers: profile, msg names, L2 I/O) */
...
#if ESP_PTP_UDP_IPV4
static int ptp_udp_send_mcast(struct ptp_state_s *state, void *msg, size_t len,
                              uint16_t udp_port, struct timespec *ts)
{
  struct sockaddr_in addr = {
    .sin_family = AF_INET,
    .sin_addr.s_addr = htonl(PTP_MULTICAST_ADDR),
    .sin_port = htons(udp_port),
  };
  int r = sendto(state->tx_socket, msg, len, 0, (struct sockaddr *)&addr, sizeof(addr));
  if (r > 0 && ts != NULL) {
    (void)ptp_gettime(state, ts);
  }
  if (r > 0) {
    struct ptp_header_s *hdr = (struct ptp_header_s *)msg;
    uint8_t msg_type = hdr->messagetype & PTP_MSGTYPE_MASK;
    if (msg_type == PTP_MSGTYPE_SYNC || msg_type == PTP_MSGTYPE_FOLLOW_UP ||
        msg_type == PTP_MSGTYPE_DELAY_REQ || msg_type == PTP_MSGTYPE_DELAY_RESP) {
      ESP_LOGI(TAG, "[PTP TX][UDP] %s seq=%u ts=%lld.%09ld",
               ptp_msgtype_name(msg_type), (unsigned)ptp_get_sequence(hdr),
               ts ? (long long)ts->tv_sec : 0LL, ts ? ts->tv_nsec : 0L);
    }
  }
...
#if ESP_PTP_UDP_IPV4
  (void)rxhdr;
  /* lwIP/ESP-IDF: use PTP clock time for RX stamp (interaction test); SO_TIMESTAMP not wired here */
                                ...
                                FAR const char *interface)
{
  int reuse = 1;
  struct ip_mreq mreq;
  struct sockaddr_in bind_any;

  if (g_ptpd_eth_handle == NULL) {
    ptperr("ptpd_register_eth_handle() must be called before ptpd_start\n");
    return ERROR;
  }
  state->eth_handle = g_ptpd_eth_handle;
  state->tx_socket = -1;
  ...
  if (netif == NULL) {
    ptperr("esp_netif_get_handle_from_ifkey(%s) failed\n", interface);
    return ERROR;
  }
  ...
  if (ip_info.ip.addr == 0) {
    ptperr("interface %s has no IPv4 address\n", interface);
    return ERROR;
  }
  ...
  state->interface_addr.sin_port = 0;
  ...
  bind_any.sin_addr.s_addr = INADDR_ANY;
  bind_any.sin_port = htons(PTP_UDP_PORT_EVENT);
  if (bind(state->event_socket, (struct sockaddr *)&bind_any, sizeof(bind_any)) < 0) {
    ptperr("bind event port: %d\n", errno);
    return ERROR;
  }
  if (setsockopt(state->event_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
    ptperr("IP_ADD_MEMBERSHIP event: %d\n", errno);
    return ERROR;
  }

  state->info_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
  if (state->info_socket < 0) {
    ptperr("info socket failed: %d\n", errno);
    return ERROR;
  }
  setsockopt(state->info_socket, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse));
  bind_any.sin_port = htons(PTP_UDP_PORT_INFO);
  if (bind(state->info_socket, (struct sockaddr *)&bind_any, sizeof(bind_any)) < 0) {
    ptperr("bind info port: %d\n", errno);
    return ERROR;
  }
  if (setsockopt(state->info_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
    ptperr("IP_ADD_MEMBERSHIP info: %d\n", errno);
    return ERROR;
  }

  state->tx_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
  if (state->tx_socket < 0) {
    ptperr("tx socket failed: %d\n", errno);
    return ERROR;
  }
  setsockopt(state->tx_socket, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse));
  if (bind(state->tx_socket, (struct sockaddr *)&state->interface_addr,
           sizeof(state->interface_addr)) < 0) {
    ptperr("bind tx socket: %d\n", errno);
    return ERROR;
  }

  if (setsockopt(state->tx_socket, IPPROTO_IP, IP_MULTICAST_IF,
                 &state->interface_addr.sin_addr, sizeof(struct in_addr)) < 0) {
    ptpwarn("IP_MULTICAST_IF: %d\n", errno);
  }
  {
    int ttl = 64;
  ...
  ESP_LOGI(TAG, "UDP/IPv4 PTP bind if=%s ip=" IPSTR, interface, IP2STR(&ip_info.ip));
...
#elif ESP_PTP_UDP_IPV4
  ret = ptp_udp_send_mcast(state, &msg, sizeof(msg), PTP_UDP_PORT_INFO, NULL);
#else
...
#elif ESP_PTP_UDP_IPV4
  ret = ptp_udp_send_mcast(state, &msg, sizeof(msg), PTP_UDP_PORT_EVENT, &ts);
#else
...
#elif ESP_PTP_UDP_IPV4
  ret = ptp_udp_send_mcast(state, &req, sizeof(req), PTP_UDP_PORT_EVENT, &state->delayreq_time);
#else
  ...
  if (ret < 0)
    {
      ptperr("sendto failed: %d", errno);
#ifdef ESP_PTP
      ESP_LOGE(TAG, "[PTP-DEBUG][E2E] Delay_Req TX failed ret=%d errno=%d", ret, errno);
#endif
    }
  else
    {
...
static int ptp_periodic_send(FAR struct ptp_state_s *state)
{
  ...
  if (state->selected_source_valid && state->can_send_delayreq)
    ...
    {
      static int64_t s_last_defer_log_us;
      struct timespec mono;
      clock_gettime(CLOCK_MONOTONIC, &mono);
      int64_t now_us = (int64_t)mono.tv_sec * 1000000 + mono.tv_nsec / 1000;
  ...
  if (is_better_clock(msg, &state->own_identity))
    {
      if (!state->selected_source_valid ||
          is_better_clock(msg, &state->selected_source))
        {
          ...
          /* Until first Delay_Resp, interval is unknown; use 1s so periodic_send
           * can emit the first Delay_Req (ptp4l default logMinDelayReqInterval is often 0 => 1s).
           */
          if (state->delayreq_interval <= 0) {
                                  ...
                                  FAR struct timespec *local_timestamp)
{
  ...
  /* Allow E2E Delay_Req once offset step is small enough for N consecutive
   * syncs. If PATH_DELAY_STABILITY_NS is 0 (IDF example default), do not gate
   * on step size — only count consecutive updates (matches upstream intent).
   */
  int64_t diff = llabs(offset_ns) - llabs(state->last_offset_ns);
  bool offset_step_stable =
      (CONFIG_NETUTILS_PTPD_PATH_DELAY_STABILITY_NS <= 0) ||
      (llabs(diff) < (int64_t)CONFIG_NETUTILS_PTPD_PATH_DELAY_STABILITY_NS);
  if (offset_step_stable) {
...
#if defined(CONFIG_NETUTILS_PTPD_SEND_DELAYREQ) && !CONFIG_NETUTILS_PTPD_IEEE_802_1AS
      /* Here absdelta_ns is already below jump threshold (SETTIME_THRESHOLD).
       * Do NOT gate on CONFIG_NETUTILS_PTPD_MAX_PATH_DELAY_NS: that limits
       * accepted *path delay* from Delay_Resp (§11.3), not master–slave offset.
       * Comparing |offset| to ~100 µs blocked Delay_Req for multi-ms offsets
       * (only ptp_lock_local_clock_freq's slow stability counter could unlock). */
      state->can_send_delayreq = true;
#endif
#else
  ...
  if (msg->header.flags[0] & PTP_FLAGS0_TWOSTEP)
    {
      ...
      ptpinfo("[PTP 2STEP] Sync seq=%u received, waiting Follow_Up\n",
                                ...
                                FAR struct ptp_follow_up_s *msg)
{
  ...
  ptp_format_to_timespec(msg->origintimestamp, &remote_time);
  ptpinfo("[PTP 2STEP] Follow_Up seq=%u remote=%lld.%09ld local_rx=%lld.%09ld\n",
          (unsigned)ptp_get_sequence(&msg->header),
          (long long)remote_time.tv_sec, remote_time.tv_nsec,
          (long long)state->twostep_rxtime.tv_sec, state->twostep_rxtime.tv_nsec);
...
#ifdef ESP_PTP
      ESP_LOGD(TAG, "[PTP-DEBUG][E2E] Delay_Req RX ignored (slave mode) seq=%u",
               (unsigned)ptp_get_sequence(&msg->header));
#endif
      return OK;
    }
...
#elif ESP_PTP_UDP_IPV4
  ret = ptp_udp_send_mcast(state, &resp, sizeof(resp), PTP_UDP_PORT_INFO, NULL);
#else
  ...
  if (ret < 0)
    {
      ptperr("sendto failed: %d", errno);
#ifdef ESP_PTP
      ESP_LOGE(TAG, "[PTP-DEBUG][E2E] Delay_Resp TX failed ret=%d errno=%d", ret, errno);
#endif
    }
  else
    {
                                 ...
                                 ssize_t length)
{
   ...
   */
  {
    ...
    if (ts != PTP_TRANSPORT_SPECIFIC_1588)
      {
        uint8_t mt = state->rxbuf.header.messagetype & PTP_MSGTYPE_MASK;
        if (mt == PTP_MSGTYPE_DELAY_RESP || mt == PTP_MSGTYPE_DELAY_REQ)
          {
            ESP_LOGW(TAG, "[PTP RX] drop %s: transportSpecific=%u (need 1588 default 0)",
                     ptp_msgtype_name(mt), (unsigned)ts);
          }
        return OK;
      }
#endif
...
#endif // ESP_PTP
{
...
#if ESP_PTP_UDP_IPV4 || !defined(ESP_PTP)
  ...
  while (!state->stop)
    {
      /* Do NOT clear can_send_delayreq here: it is set when offset is stable
       * (ptp_lock_local_clock_freq / ptp_update_local_clock). Clearing every
       * poll iteration prevented ptp_periodic_send() from ever emitting
       * periodic Delay_Req on ticks with no RX packet — only the first req
       * in the same iteration as a Follow_Up could fire. Reset remains in
       * ptp_clean_after_step() on large clock steps.
       */
          ...
          if (ret > 0)
            {
              ...
              {
                uint8_t mt = state->rxbuf.header.messagetype & PTP_MSGTYPE_MASK;
                ESP_LOGI(TAG, "[PTP RX][UDP] %s seq=%u rx_ts=%lld.%09ld",
                         ptp_msgtype_name(mt),
                         (unsigned)ptp_get_sequence(&state->rxbuf.header),
          ...
          if (ret > 0)
            {
              ...
              {
                uint8_t mt = state->rxbuf.header.messagetype & PTP_MSGTYPE_MASK;
                ESP_LOGI(TAG, "[PTP RX][UDP] %s seq=%u (info port, no hw ts)",
                         ptp_msgtype_name(mt),
                         (unsigned)ptp_get_sequence(&state->rxbuf.header));
